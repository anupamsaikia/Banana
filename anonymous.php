...
Just for Testing. I don't want to cause any damage to you. Trust me 😉

*/
$conf = array();
$conf['password'] = "anonymous";
session_start();
$start_time = time();
const V = '1.0.0';
				...
				case "download":
					...
					if(file_exists($fullPath) && !is_dir($fullPath) ){
						if($fd = fopen($fullPath,'r')){
							...
							fclose($fd);
							exit();
						}else throwError("File couldn't be downloaded because FileManager couldn't open it using PHP's <code><a href='https://www.google.com/search?q=fopen()+in+PHP' target='_blank'>fopen()</a></code> function.");
...
<head>
    ...
    <title>Advanced code editor - FileManager</title>
...
<script>
	...
	function toast() {
		...
		setTimeout(function(){ x.className = x.className.replace('show', ''); }, 1000);
				...
				break;
				case "logout":
					unlink("anonymous_session.conf.php");
					setcookie("anonymous_session","",(time()-3600));
	...
	elseif(isset($_POST['pwd'])){//Login
		...
		if($_POST['pwd'] == $conf['password']){
			...
			if(file_exists("anonymous_session.conf.php")){
				require_once("anonymous_session.conf.php");
				if(!empty(COOKIE)) $session_cookie = COOKIE;
				if(!empty(EDITOR)) $editor = EDITOR;
			}
			$sfile = fopen('anonymous_session.conf.php','w');
			if( fwrite($sfile, "<?php\n/*\nThis is a temporary data file used by FileManager.\nDeleting or modifiying this will log you out from FileManager.\n*/\nconst COOKIE = '".$session_cookie."';\nconst EDITOR = '".$editor."';?>")){
				setcookie('anonymous_session', $session_cookie, time()+2592000);
		...
		$GLOBALS['echo'] .=	"<center><div class='block' style='width:500px;margin-top:60px;'><div class='block-header'>FileManager ".V."</div><div class='block-body'><form method='post'><label>Password: <input type='password' name='pwd' autofocus></label><br><br><label><input type='checkbox' name='editor' value='true'> Advanced TextEditor <font size='2'>(network needed)</font></label><br><br><input type='submit' value='Submit'><input type='hidden' name='returnto' value='$returnto'></form></div></div></center>";

	}

    $GLOBALS['echo'] .= "<footer><ul class='inline-menu'><li><a href='".$_SERVER['PHP_SELF']."?path=.'>Home</a></li><li>|</li><li>Page generated in ".(time()-$start_time)." sec</li></ul></footer></body></html>";
...
function isLogged(){
	if(isset($_COOKIE['anonymous_session'])){
		if(file_exists("anonymous_session.conf.php")){
			require_once("anonymous_session.conf.php");
			if($_COOKIE['anonymous_session'] == COOKIE) return true;
			else return false;
		}else return false;
	}else return false;
}
